...
1) SET SYSTEM ENVIRONMENT for the following :
...
2) Download code base from BitBucket repository (We have updated library's and Java Version)
3) Go to Mysql and create an user = "linshare" password = "linshare" 
	a) Create two schemas  named 1) linshare 2) linshare_data 
	b) Import self contained file  (eg:- linshare_schemas.sql) to the db "linshare" only.

4)	After that go to workspace(Eclipse workspace) and create a repository ‘configurations’. 
	...
	And also  copy the log4j.properties from the source code(\src\main\resources) and put it into my_conf1.
	
	And create a folder with name ‘projects’ under my_conf1 and put your linshare project there also with name linshare.

	


5)	Go to Eclipse and configure jetty and maven  : 
		
		For maven set goal : jetty:run
					  profile : h2,local
		
		Set environement Variable : LINSHARE_HOME  Value : ${workspace}  eg:- H:\MyWorkspace    (Eclpise Workspace) for both maven and jetty
...
6) Install apache http server.
 HYPERLINK "http://www.apachehaus.com/cgi-bin/download.plx?dli=kVFbQRWaNNjTqJ1KatmWSBlVOpkVFVFdUNzayEWQ"
...
7) Download 
 HYPERLINK "http://download.linshare.org/service/local/artifact/maven/content?r=linshare-releases&g=org.linagora.linshare&a=linshare-ui-user&v=2.0.0&p=tar.bz2"
